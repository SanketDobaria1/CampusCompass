...
router
  ...
  .post(async (req, res) =>{
    ...
    try{        
      req.params.id = validation.checkId(req.params.id, 'Id URL Parameter');
      if (typeof data.room_number !== 'number') throw `Room number must be a Number !`
      if (typeof data.capacity !== 'number') throw `Room capacity must be a Number !`
      if (typeof data.floor_number !== 'number') throw `Floor number must be a Number !`
    ...
    try {
      ...
      await roomsData.create(req.params.id, room_number, capacity, floor_number, type);
