...
router.route('/')
  ...
  .post(async (req, res) => {
    ...
    try{
      ...
      data.operating_hours = validation.checkStringArray(data.operating_hours, 'Operating Hours')
    } catch (e){
    ...
    try {
        ...
        const newLocation = await locationsData.create(name, desc, type, operating_hours);
...
router
  ...
  .put(async (req, res) => {
    ...
    try{
      req.params.id = validation.checkId(req.params.id, 'Id URL Parameter');
      updatedData.name = validation.checkString(updatedData.name, 'Location Name');
      updatedData.desc = validation.checkString(updatedData.desc, 'Description')
      updatedData.type = validation.checkString(updatedData.type, 'Location Type')
      updatedData.operating_hours = validation.checkStringArray(updatedData.operating_hours, 'Operating Hours')
    } catch (e){
      return res.status(400).json({error: e})
    }
    ...
    try {
        ...
        const updatedLocation = await locationsData.update(req.params.id, name, desc, type, operating_hours);
